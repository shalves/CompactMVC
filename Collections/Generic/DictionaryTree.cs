namespace System.Collections.Generic
{
    ...
    public class DictionaryTree<K, V> : IEnumerable<DictionaryTree<K, V>>
    {
        ...
        public void EachAllBranches(Func<DictionaryTree<K, V>, bool> func)
        {
            foreach (var branche in Branches)
            {
                if (!func(branche.Value)) break;
                if (branche.Value.LinealBranchesCount > 0) branche.Value.EachAllBranches(func);
            }
        }
